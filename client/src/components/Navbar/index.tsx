...
export default function Navbar({ children }: DashboardProps) {
  ...
  const userRoutes = [
    { text: "Дом", icon: IoHomeOutline, path: "/" },
    {
      text: "Appointments",
      icon: IoDocumentTextOutline,
      path: "/appointments",
    },
    { text: "Подать заявку на врача", icon: FaUserDoctor, path: "/apply-doctor" },
    { text: "Профиль", icon: FaRegCircleUser, path: `/profile/${userId}` },
  ];
  ...
  const adminRoutes = [
    ...
    { text: "Врачи", icon: FaUserDoctor, path: "/doctors" },
    { text: "Профиль", icon: FaRegCircleUser, path: `/profile/${userId}` },
  ];

  const routes = isAdmin ? adminRoutes : isDoctor ? doctorRoutes : userRoutes;
  const chipLabel = isAdmin ? "Админ" : isDoctor ? "Доктор" : "Пользователь";
